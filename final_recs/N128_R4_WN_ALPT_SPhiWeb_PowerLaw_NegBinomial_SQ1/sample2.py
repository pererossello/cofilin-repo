...
lastrecfold = os.path.join(this_file_dir, f"00CH00")

savefold = os.path.join(lastrecfold, f"S01")
make_and_check_dir(savefold)

figfold = os.path.join(savefold, "figures")
make_and_check_dir(figfold)
resultsfold = os.path.join(savefold, "results")
make_and_check_dir(resultsfold)
...
for i in range(chunks):
    ...
    sample_path = os.path.join(resultsfold, f"{i:04d}.hdf5")
    ...
    with h5py.File(sample_path, "r") as f:
        q_in = f["sample"]["q"][:]
        alpha_in = f["sample"]["alpha"][:]
        beta_in = f["sample"]["beta"][:]
